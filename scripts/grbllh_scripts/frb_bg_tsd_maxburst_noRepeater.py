...
# assign 'sigma' to events
#bg['sigma'] = np.array([0.035]*len(zenith))
#mc['sigma'] = np.array([0.035]*len(mc['zenith']))
# try energy-dependent sigma, check response in different spectra
#for i in range(len(mc['sigma'])):
#    if mc['true_energy'][i] > 100000.: mc['sigma'][i] = 0.01
#    elif mc['true_energy'][i] > 10000.: mc['sigma'][i] = 0.05
#    elif mc['true_energy'][i] > 1000.: mc['sigma'][i] = 0.1
...
t1 = time.time()
tsd_bg_tw = grbllh.do_trials(N, [analysis], llh_type=grbllh.LlhType.max_source, mu=0., seed=options.randomseed)
#print "Trials complete!\n%i seconds to complete 1e6 trials"%int(time.time()-t1)
...
else: cache.save([tsd_bg_tw], "/data/user/sfahey/FRB/L2_Analysis/tsd/bg/tsd_maxburst_seed%s_dT%.3f"%(str(options.randomseed).zfill(3), dT))
