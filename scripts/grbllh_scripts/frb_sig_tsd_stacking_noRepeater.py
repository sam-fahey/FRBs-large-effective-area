...
thresh_file = '/data/user/sfahey/FRB/L2_Analysis/tsd/bg/thresholds_stacking.pkl'
...
print len(mc['zenith'])
mcmask1, mcmask2 = mc['zenith'] > -1, mc['energy'] > -1
mcmask = mcmask1 * mcmask2
for key in mc.keys():
    mc[key] = mc[key][mcmask]
print len(mc['zenith'])
...
print "Loading background TSD for thresholds..."
...
mu = grbllh.find_sig_thresh (thresh[0], 0.9, 1e4, [analysis],
                             mu_bottom = 1e-3,
                             mu_top = 1e-2,
                             use_diffsig=False,
                             log=True,
                             llh_type=grbllh.LlhType.overall,
                             full_output=True)
print ('Point source thrower E^-2 sensitivity: {0:.4f} (GeV cm^-2)'.format (
    mu['mu'] * len (analysis.bg_sources)))
cache.save(mu, "/data/user/sfahey/FRB/L2_Analysis/tsd/sig/tsd_sens_stacking_E-%.1f_seed%s_dT%s"%(gamma, options.randomseed, options.centertime))

mu = grbllh.find_sig_thresh (thresh[5], 0.9, 1e4, [analysis],
                             mu_bottom = mu_bottom,
                             mu_top = mu_top,
                             use_diffsig=False,
                             log=True,
                             llh_type=grbllh.LlhType.overall,
                             full_output=True)
print ('Point source thrower 5\sigma 90% E^-2 discovery potential: {0:.4f} (GeV cm^-2)'.format (
    mu['mu'] * len (analysis.bg_sources)))
cache.save(mu, "/data/user/sfahey/FRB/L2_Analysis/tsd/sig/tsd_dp5sig_stacking_E-%.1f_seed%s_dT%s"%(gamma, options.randomseed, options.centertime))
...
mu = grbllh.find_sig_thresh (thresh[4], 0.9, 1e4, [analysis],
                             ...
                             llh_type=grbllh.LlhType.overall,
                             full_output=True)
print ('Point source thrower 4\sigma 90% E^-2 discovery potential: {0:.4f} (GeV cm^-2)'.format (
    mu['mu'] * len (analysis.bg_sources)))
cache.save(mu, "/data/user/sfahey/FRB/L2_Analysis/tsd/sig/tsd_dp4sig_stacking_E-%.1f_seed%s_dT%s"%(gamma, options.randomseed, options.centertime))

mu = grbllh.find_sig_thresh (thresh[3], 0.9, 1e4, [analysis],
                             mu_bottom = mu_bottom,
                             mu_top = mu_top,
                             use_diffsig=False,
                             log=True,
                             llh_type=grbllh.LlhType.overall,
                             full_output=True)
print ('Point source thrower 3\sigma 90% E^-2 discovery potential: {0:.4f} (GeV cm^-2)'.format (
    mu['mu'] * len (analysis.bg_sources)))
cache.save(mu, "/data/user/sfahey/FRB/L2_Analysis/tsd/sig/tsd_dp3sig_stacking_E-%.1f_seed%s_dT%s"%(gamma, options.randomseed, options.centertime))
